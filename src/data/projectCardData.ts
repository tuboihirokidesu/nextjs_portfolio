import { ICardData } from '../interfaces'

const projectCardData: ICardData[] = [
  {
    id: '0',
    src:
      'https://images.unsplash.com/photo-1521295121783-8a321d551ad2?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1050&q=80',
    text: 'World Rank',
    label: 'React Hooks',
    path: './project/world-ranks'
  },
  {
    id: '1',
    src:
      'https://images.unsplash.com/photo-1605377347958-e8bd4c00ba1d?ixlib=rb-1.2.1&ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&auto=format&fit=crop&w=700&q=80',
    text: 'コロナの情勢',
    label: 'Axios',
    path: './project/corona'
  },
  {
    id: '2',
    src:
      'https://images.unsplash.com/photo-1557821552-17105176677c?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1189&q=80',
    text: 'ショッピングカート',
    label: 'JavaScript',
    path: './project/shopping-cart'
  },
  {
    id: '3',
    src:
      'https://images.unsplash.com/photo-1609266378844-4a8af6d72fab?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80',
    text: 'Appleカレンダー',
    label: 'moment.js',
    path: './project/calender'
  },
  {
    id: '4',
    src:
      'https://images.unsplash.com/photo-1587620962725-abab7fe55159?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1189&q=80',
    text: 'World Ranks',
    label: 'CSS',
    path: './project/world-ranks'
  }
]

export default projectCardData
